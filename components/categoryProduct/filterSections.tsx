  ...
  const toggleFilter = (filterType: string, value: string) => {
    ...
    setFilterSelected((prevFilterSelected) => ({
      ...
      [filterType]: prevFilterSelected[filterType].includes(value)
        ? prevFilterSelected[filterType].filter(
      ...
      <div className="w-full flex-[0.2] flex lg:hidden gap-4  ">
        ...
        <Modal isOpen={isFilterByPrice} setOpen={setIsFilterByPrice}>
          <div className="bg-white sm:w-[60%] w-[100%] absolute top-0 left-0  h-screen   flex-col">
            <div className="w-full text-center text-lg font-semibold border-b py-4 relative ">
              Filter
            </div>
      ...
      <div className="hidden lg:flex flex-col flex-[0.25] filter-border mt-2 pb-5 border ">
        <div className="px-3">
          <div className="flex justify-between items-center   py-3">
            <h4 className="font-semibold text-lg text-secondary ">Filter</h4>
          </div>
        </div>
          ...
          filtersData.map((filter: any, idx: number) => (
            <div key={idx}>
              ...
              {isFiltersOpen[filter.name] && (
                <div
                  className={`${
                    filter.name === "colour" ? "flex flex-wrap" : "grid"
                  }  grid-cols-3 items-center gap-5  pt-4 px-3`}
                >
                  {filter.values.map((value: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-2">
                      <div
                        ...
                        className={`h-[20px] w-[20px] flex items-center justify-center border p-1 ${
                          filter.name === "colour" && "rounded-full"
                        } 
                        ${
                          selectedSizes[value] &&
                          filter.name === "colour" &&
                        ...
                        {selectedSizes[value] && (
                          <div
                            className={`h-full w-full  ${
                              selectedSizes[value] && filter.name === "colour"
                                ? "bg-none"
                                : "bg-primary"
                            }  `}
                            // style={{
        ...
        <p className="font-semibold text-sm text-secondary py-4 px-3">Select Price Range</p>
